...
### 1. Schema Validation — Rejection Cases

Each of the following inputs must cause schema validation to exit non-zero with a structured error message referencing the correct VR rule ID:

| Test | Input | Expected VR rule |
|------|-------|-----------------|
| VR-01501 | `changed` contains an entity not in parent canonical model | VR-01501 |
| VR-01502 | `removed.<entity>.fields` contains a field not in parent entity | VR-01502 |
| VR-01503 | `added.<entity>.relationships` references a target that does not exist | VR-01503 |
| VR-01504 | Same entity name appears in both `added` and `changed` | VR-01504 |
| VR-01505 | Unknown top-level key (e.g., `modified`) is present | VR-01505 |
| VR-01506 | `added` contains an entity that already exists in parent canonical model | VR-01506 |
| VR-01507 | `removed` contains an entity not in parent canonical model | VR-01507 |

Fixture files for each invalid input should be placed in `tests/smoke/fixtures/invalid/`.

### 2. Parser — Correctness Cases
...
Fixture files for valid inputs and expected canonical JSON outputs should be placed in `tests/smoke/fixtures/valid/`.

### 3. Renderer — Output Correctness Cases
...
Expected markdown output fixtures should be placed in `tests/smoke/fixtures/expected-md/`.
...
├── fixtures/
│   ├── invalid/
│   │   ├── vr-01501-changed-unknown-entity.yaml
│   │   ├── vr-01502-removed-unknown-field.yaml
│   │   ├── vr-01503-dangling-relationship-target.yaml
│   │   ├── vr-01504-duplicate-entity-across-sections.yaml
│   │   ├── vr-01505-unknown-top-level-key.yaml
│   │   ├── vr-01506-added-entity-already-exists.yaml
│   │   └── vr-01507-removed-entity-not-in-parent.yaml
│   ├── valid/
